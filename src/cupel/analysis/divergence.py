...
def measure_key(release: str, vdir: str, param_set: str, tc_id: int, reason: str,
    ...
    return KeyMeasurement(
        release=release, vdir=vdir, param_set=param_set, tc_id=tc_id, reason=reason,
...
def measure_release(release: str, projection: dict,
                    vdir: str = "") -> list[KeyMeasurement]:
    """Every negative encapsulationKeyCheck case in one vector directory.

    `vdir` is carried through so a reader can tell the two encapsulation
    directories apart. They hold 15 invalid keys each, all distinct, and reading
    only one of them undercounts the population by half.
    """
    out = []
    for group in projection.get("testGroups", []):
        ...
        for test in group.get("tests", []):
            ...
            out.append(measure_key(
                release, vdir, param_set, test.get("tcId", -1),
...
def summarise(keys: list[KeyMeasurement],
    ...
    for release in sorted(by_release):
        ...
        out.append({
            ...
            "vector_dirs": sorted({k.vdir for k in ks if k.vdir}),
...
# ---------------------------------------------------------------------------
# How close the corpus gets to the bound
# ---------------------------------------------------------------------------

def z_margins(projection: dict, release: str, reason: str) -> list[dict]:
    """Per case: the largest |z| coefficient, against the bound it must clear.

    A column of zeros says no case violates the bound. It does not say whether
    the corpus comes anywhere near it, and those are different findings. A set
    whose maximum sits three units under the bound was aimed at it and missed;
    a set an order of magnitude below it was never aiming.

    Added after review: the vectors at the newest pin were regenerated by
    something outside the repository, so the landing analysis computed from the
    producer source does not describe them, and nothing else in this project
    did either.
    """
    from ..predicates.mldsa import decode_z, params_of

    out = []
    for group in projection.get("testGroups", []):
        for test in group.get("tests", []):
            if reason and test.get("reason") != reason:
                continue
            ...
            peak = max(abs(c) for c in decode_z(sig, p))
            out.append({
                "schema": "zmargin/1",
                "release": release,
                "param_set": group.get("parameterSet"),
...
def summarise_margins(rows: list[dict]) -> list[dict]:
    by: dict[tuple, list[dict]] = {}
    for r in rows:
        by.setdefault((r["release"], r["param_set"]), []).append(r)
    out = []
    for (release, ps), rs in sorted(by.items()):
        margins = [r["margin"] for r in rs]
        out.append({
            "schema": "zmargin-summary/1",
            "release": release,
            "param_set": ps,
            "n_cases": len(rs),
            "bound": rs[0]["bound"],
            "max_abs_z_min": min(r["max_abs_z"] for r in rs),
            "max_abs_z_max": max(r["max_abs_z"] for r in rs),
            "closest_margin": min(margins),
            "n_violating": sum(1 for r in rs if r["violates"]),
        })
    return out
